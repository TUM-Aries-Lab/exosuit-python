...
from motor_python.can_utils import ensure_can_interface
...
class Exosuit:
    ...
    def __init__(self, config: ExosuitConfig) -> None:
        ...
        if self.config.mock_devices or self.config.test_gpio:
            ...
            # alone, since bringing a working link down and up can drop a motor
            # into BUS-OFF.
            #
            # A failure is logged, not raised: _initialize_motors reports the
            # real state of the link a moment later, and a suit that cannot
            # reach its motors should say so once, in its own words.
