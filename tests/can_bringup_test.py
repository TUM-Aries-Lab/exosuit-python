"""Tests for the CAN interface bring-up.
...
from exosuit_python import can_bringup
...
def _pretend_ip_exists(monkeypatch):
    ...
    monkeypatch.setattr(can_bringup.shutil, "which", lambda _: "/usr/bin/ip")


def test_reads_an_interface_that_is_up():
    """Real Jetson output for a healthy can0."""
    exists, is_up, bitrate = can_bringup.read_interface(CONFIG)
...
def _default_ip_output(monkeypatch):
    ...
    monkeypatch.setattr(can_bringup, "_run", _ip_returning(UP_AT_1M))
...
def test_reads_an_interface_that_is_down(monkeypatch):
    ...
    monkeypatch.setattr(can_bringup, "_run", _ip_returning(DOWN))
...
def test_a_healthy_interface_is_left_completely_alone(monkeypatch):
    ...
    monkeypatch.setattr(can_bringup, "_run", recording)

    assert can_bringup.ensure_can_interface(CONFIG) is True
...
def test_a_down_interface_is_brought_up(monkeypatch):
    ...
    monkeypatch.setattr(can_bringup, "_run", fake)
    ...
    assert str(CONFIG.bitrate) in configuring[1]
...
def test_a_missing_interface_is_reported_not_created(monkeypatch):
    ...
    monkeypatch.setattr(can_bringup, "_run", _ip_returning("", returncode=1))

    assert can_bringup.ensure_can_interface(CONFIG) is False
...
def test_it_does_nothing_off_linux(monkeypatch):
    ...
    monkeypatch.setattr(can_bringup.shutil, "which", lambda _: None)
...
def test_a_hanging_command_does_not_propagate(monkeypatch):
    ...
    monkeypatch.setattr(can_bringup.subprocess, "run", explode)
